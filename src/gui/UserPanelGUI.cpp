...
{
    if (m_focus == F_NONE) m_focus = (delta > 0) ? F_LOGIN : F_BACK;
    else                   m_focus = (m_focus + delta + F_COUNT) % F_COUNT;
}

void UserPanelScreen::activate()
{
    if      (m_focus == F_LOGIN)    m_app->push(std::make_unique<UserLoginScreen>(m_font));
    else if (m_focus == F_REGISTER) m_app->push(std::make_unique<UserRegistrationScreen>(m_font));
...
void UserPanelScreen::handleEvent(const sf::Event& event)
{
    ...
    if (event.is<sf::Event::MouseMoved>()) m_focus = F_NONE;
    ...
    if (const auto* kp = event.getIf<sf::Event::KeyPressed>())
    {
        ...
        switch (kp->code)
        {
            case Key::Enter:
                ...
                if (m_focus == F_NONE) m_focus = F_LOGIN;
                activate();
                break;
