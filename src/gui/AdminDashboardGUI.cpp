...
void AdminDashboardScreen::saveBus()
{
    ...
    try
    {
        seats = std::stoi(m_busSeatsBox.getText());
    }
    catch (...)
    {
        seats = -1;
...
void AdminDashboardScreen::handleEvent(const sf::Event &event)
{
    ...
    if (event.is<sf::Event::MouseButtonPressed>())
    {
        ...
        if (inContent)
        {
            ...
            {
                if (m_uniCodeBox.getBounds().contains(m_mouse))
                    m_focusField = 0;
                if (m_uniNameBox.getBounds().contains(m_mouse))
                    m_focusField = 1;
                if (saveUniRect.contains(m_mouse))
                {
                    saveUniversity();
                    return;
                }
                if (cancelUniRect.contains(m_mouse))
                {
                    cancelUniversity();
                    return;
                }
            ...
            if (m_state == ADD_BUS)
            {
                if (m_busIdBox.getBounds().contains(m_mouse))
                    m_focusField = 0;
                if (m_busNameBox.getBounds().contains(m_mouse))
                    m_focusField = 1;
                if (m_busUniBox.getBounds().contains(m_mouse))
                    m_focusField = 2;
                if (m_busSeatsBox.getBounds().contains(m_mouse))
                    m_focusField = 3;
                if (m_busRouteBox.getBounds().contains(m_mouse))
                    m_focusField = 4;
                if (saveBusRect.contains(m_mouse))
                {
                    saveBus();
                    return;
                }
                if (cancelBusRect.contains(m_mouse))
                {
                    cancelBus();
                    return;
                }
    ...
    if (const auto *kp = event.getIf<sf::Event::KeyPressed>())
    {
        ...
        if (m_state == ADD_UNIVERSITY || m_state == ADD_BUS)
        {
            ...
            case Key::Enter:
                ...
                if (m_focusField == FIELD_NONE)
                    m_focusField = 0;
                else if (m_focusField < fields - 1)
                    ++m_focusField;
                else if (m_focusField == cancAt)
                {
                    if (uni)
                        cancelUniversity();
                    else
        ...
        event.is<sf::Event::MouseButtonPressed>())
    {
        ...
        {
            if (m_focusField == 0)
                m_uniCodeBox.handleEvent(event);
            else
                m_uniNameBox.handleEvent(event);
        }
        if (m_state == ADD_BUS)
        {
            switch (m_focusField)
            {
            case 0:
                m_busIdBox.handleEvent(event);
                break;
            case 1:
                m_busNameBox.handleEvent(event);
                break;
            case 2:
                m_busUniBox.handleEvent(event);
                break;
            case 3:
                m_busSeatsBox.handleEvent(event);
                break;
            case 4:
                m_busRouteBox.handleEvent(event);
                break;
            default:
                break;
            }
...
void AdminDashboardScreen::drawSidebar(sf::RenderTarget &target)
{
    ...
    Theme::drawTextHCentered(target, m_font, "Bus Tracker", Theme::Type::SUBTITLE,
                             Theme::ACCENT_HOVER, SW * 0.5f, 8.f, sf::Text::Bold);
    Theme::drawTextHCentered(target, m_font, "ADMIN", Theme::Type::CAPTION,
                             Theme::TEXT_MUTED, SW * 0.5f, 33.f, sf::Text::Bold);
...
void AdminDashboardScreen::drawHeader(sf::RenderTarget &target)
{
    ...
    else if (m_state == VIEW_BUSES || m_state == ADD_BUS)
        pageTitle = "Buses";
...
void AdminDashboardScreen::draw(sf::RenderTarget &target)
{
    ...
    else if (m_state == VIEW_UNIVERSITIES)
    {
        ...
        for (int i = 0; i < static_cast<int>(m_unis.size()); ++i)
        {
            ...
            if (cy + UCARD_H < HH || cy > wh)
                continue;

            bool sel = (i == m_selIdx);
            bool hov = inContent && mx >= CARD_X && mx < CARD_X + CARD_W &&
                       my >= cy && my < cy + UCARD_H;
            sf::Color bg = sel ? Theme::ITEM_SELECTED : hov ? Theme::ITEM_HOVER
                                                            : Theme::ITEM_BG;
    ...
    else if (m_state == VIEW_BUSES)
    {
        ...
        for (int i = 0; i < static_cast<int>(m_buses.size()); ++i)
        {
            ...
            string secondary = m_buses[i].getUniversityCode() + "  ·  " + to_string(m_buses[i].getTotalSeats()) + " seats";
