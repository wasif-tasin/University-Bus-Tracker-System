...
void UserDashboardScreen::onEnter()
{
    ...
    if (m_state == USER_VIEW_UNIVERSITIES)
        m_unis = m_user.getUniversities();
    if (m_state == USER_VIEW_BUSES)
        m_buses = m_user.getBuses();
}
...
void UserDashboardScreen::goTo(State next)
{
    ...
    if (next == USER_VIEW_UNIVERSITIES)
        m_unis = m_user.getUniversities();
    if (next == USER_VIEW_BUSES)
        m_buses = m_user.getBuses();
}
...
void UserDashboardScreen::handleEvent(const sf::Event &event)
{
        ...
        event.is<sf::Event::MouseButtonPressed>())
    {
        ...
        if (m_state == USER_SEARCH_BUS)
            m_busIdBox.handleEvent(event);
        if (m_state == USER_SEARCH_BY_STOP)
            m_stopBox.handleEvent(event);
    }
}
...
void UserDashboardScreen::drawHeader(sf::RenderTarget &target)
{
    ...
    if (m_state == USER_VIEW_UNIVERSITIES)
        ...
        pageTitle = "Buses by University";
    else if (m_state == USER_SEARCH_BUS)
        pageTitle = "Search Bus";
    else if (m_state == USER_SEARCH_BY_STOP)
        pageTitle = "Search by Stop";
